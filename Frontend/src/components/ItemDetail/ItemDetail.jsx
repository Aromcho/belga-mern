...
const ItemDetail = ({ property }) => {
  ...
  const tags = property.tags[0].name;
  ...
  const sliderSettings = {
    ...
    infinite: true,
    speed: 500,
    slidesToShow: 1.5, // Muestra 1.5 imágenes
    slidesToScroll: 1,
    nextArrow: <NextArrow />,
    prevArrow: <PrevArrow />,
    centerMode: true, // Mantén la imagen central destacada
    autoplay: true,
    autoplaySpeed: 3000,
    responsive: [
      {
        ...
        settings: {
          slidesToShow: 1.2, // Ajusta a 1.2 imágenes en pantallas móviles
          slidesToScroll: 1,
        }
      },
      {
        breakpoint: 480, // Para pantallas móviles muy pequeñas
        settings: {
          slidesToShow: 1, // Muestra solo una imagen en móviles pequeños
          slidesToScroll: 1
        }
      }
    ]
  ...
  <div className="p-3 bg-white rounded-3 shadow">
    <Button as={Link} to="/propertylist" variant="primary" className="mt-3 w-100 custom-button">
      <FaArrowLeft className="me-2" /> {/* Icono con margen a la derecha */}
      Volver a la lista
    </Button>
    ...
    <div className="info-details d-flex flex-column gap-4">
      {propertyApi.disposition && (
        <div className="info-item-b p-3 d-flex align-items-center rounded shadow hover-effect">
          <FaRulerCombined className="icon me-3 "  />
          <p className="mb-0" ><strong>Disposición: </strong>{propertyApi.disposition}</p>
        </div>
      )}
      {propertyApi.orientation && (
        <div className="info-item-b p-3 d-flex align-items-center rounded shadow hover-effect">
          <FaCompass className="icon me-3 " />
          <p className="mb-0" ><strong>Orientación: </strong>{propertyApi.orientation}</p>
        </div>
      )}
      {propertyApi.property_condition && (
        <div className="info-item-b p-3 d-flex align-items-center rounded shadow hover-effect">
          <FaBuilding className="icon me-3 "  />
          <p className="mb-0" ><strong>Condición: </strong>{propertyApi.property_condition}</p>
        </div>
      )}
    </div>
  </Row>
